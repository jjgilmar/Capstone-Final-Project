...
## Original Data=
```{r}
pacman::p_load(nflfastR, dplyr, tidyr, ggplot2, scales)
...
down4 <- down4|>
  ...
  mutate(
    ...
    # Rate — guard against division by zero on the first attempt
...
down4 <- down4 |> 
  mutate(
    drive_play_count = ifelse(
      game_id == "2019_15_ATL_SF" & play_id == 678,
      11,
      drive_play_count
    ),
    drive_play_count = ifelse(
      game_id == "2022_17_CLE_WAS" & play_id == 1580,
      21,
      drive_play_count
    ),
    drive_play_count = ifelse(
      game_id == "2018_02_IND_WAS" & play_id == 4031,
      12,
      drive_play_count
    ),
    drive_play_count = ifelse(
      game_id == "2016_15_JAX_HOU" & play_id == 1636,
      12,
      drive_play_count
    )
  )
...
convert_drive_start <- function(drive_start, posteam) {
  # Split "TEAM YY" into parts
  if(drive_start == 50)
    {return(drive_start)
  } else {
  
  parts <- strsplit(drive_start, " ")[[1]]
  team  <- parts[1]
  yards <- as.integer(parts[2])
  
  # If the team in the field position matches the possession team,
  # they're on their own side → distance to opponent's end zone = 100 - yards
  # Otherwise they're in opponent territory → distance = yards
  if (team == posteam) {
    return(100L - yards)
  } else {
    return(yards)
...
down4 <- down4 |> 
  filter(!is.na(drive_start_yard_line))|>
  mutate(
    drive_start_100 = mapply(convert_drive_start, drive_start_yard_line, posteam),
    drive_start_100 = as.integer(drive_start_100)) 
...
down4 <- down4 |>  
  mutate(
    ...
    top_seconds = as.integer(lubridate::ms(drive_time_of_possession), units = "secs"),
    posteam_winning = ifelse(score_differential > 0, 1, 0),
    play_in_rz = ifelse(yardline_100 <= 20, 1, 0),
    drive_start_turnover = ifelse( drive_start_transition %in% c('INTERCEPTION', 'MUFFED_PUNT', 'DOWNS', 'MUFFED_KICKOFF'), 1, 0),
    drive_yards = drive_start_100 - yardline_100
  )
```
